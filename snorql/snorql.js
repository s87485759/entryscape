...
function Snorql() {
    ...
    this._endpoint = 'http://knowone.csc.kth.se/sparql/ariadne-big';
    // modify these to your likeing
    this._poweredByLink = 'http://www.openrdf.org';
    this._poweredByLabel = 'Sesame';
    ...
    this.start = function() {
        ...
        if (!queryString) {
            ...
            document.getElementById('querytext').value = 'SELECT ?uri ?title ?desc ?context WHERE {\n  GRAPH ?context {\n    ?uri dcterms:title ?title ;\n      dcterms:audience lomvoc:IntendedEndUserRole-learner ;\n      lom:copyrightAndOtherRestrictions "false"^^xsd:boolean ;\n      lom:status lomvoc:Status-final ;\n      dcterms:description ?b .\n          ?b rdf:value ?desc .\n    FILTER(\n      langMatches(lang(?title),"en") &&\n      langMatches(lang(?desc),"en") &&\n      regex(str(?title), "^organic", "i")\n    )\n  }\n}\nLIMIT 50';
