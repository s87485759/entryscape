...
dojo.provide("hnetfolio.editor.UserEditor");
...
dojo.declare("hnetfolio.editor.UserEditor", [folio.editor.UserEditor], {
	...
	templatePath: dojo.moduleUrl("hnetfolio", "editor/UserEditorTemplate.html"),
	...
	startup: function() {
	    ...
	    dojo.connect(this.UserProfileTabs, "selectChild", dojo.hitch(this, function(child){
			if(!this.CompProfileCreated && child.dojoAttachPoint === 'CPContentTab'){
	...
	_fixCompetenceTab :function(){
			...
			if(hasComp){
				...
				this.store.loadEntry(entryObj, {},  dojo.hitch(this, function(result) {
					...
					this.CPContentPane.attr('content',this.CPPlain);
					this.CPPlain.setGraph(result.getLocalMetadata(), 'http://rforms.confolio.org/formulator/formlet/HnetPPSec1');//'http://localhost:8080/formulator/formlet/HnetPPSec1');	
				    this.CPPlain.recommendedChange(); //Sets the editor to include the Recommended elements
				}),
				dojo.hitch(this, function(result){
					this.CPContentPane.attr('content',"Failed getting Profile");
				}));      	
			}else{
				...
				dojo.hitch(function(result){
					this.CPContentPane.attr('content',"Failed to create Profile");
				}));
			}

	},
	changeClicked:function () {
	    if(this.CompetenceEntry){
			var CPGraph = this.CPPlain.getGraph();
			CPGraph.add(this.CompetenceEntry.getResourceUri(), "sc:aboutPerson", {"@id": this.userEntry.getResourceUri()});
			var tmp2 = {metadata : CPGraph.getRoot()};
			this.CompetenceEntry.getContext().communicator.saveJSON(this.CompetenceEntry.getLocalMetadataUri(),tmp2);	
	        this.CompetenceEntry.setRefreshNeeded(); 
		}
		this.inherited("changeClicked", arguments);
	}
});
/*dojo.declare("hnetfolio.editor.UserEditor", [dijit._Widget, dijit._Templated, folio.editor.ResourceEditor], {
	attributeMap: dojo.mixin(dojo.clone(dijit.layout.ContentPane.prototype.attributeMap), {
		userNameLabel: {node: "userNameLabel", type: "innerHTML"},
		passwdLabel: {node: "passwdLabel", type: "innerHTML"},
		passwd2Label: {node: "passwd2Label", type: "innerHTML"},
	...
	postCreate: function() {
		...
		this.languageSelect.store = this.langStore;
	},
	startup: function() {
		this.inherited("startup", arguments);
	...
	localize: function() {
		...
		if (this.UserProfileTabs.tablist.hasChildren()) {
			this.UserProfileTabs.tablist.getChildren()[0].attr("label", this.resourceBundle.accountLabel);
			this.UserProfileTabs.tablist.getChildren()[1].attr("label", this.resourceBundle.profileLabel);
		}
	},
	setApplication: function(application) {
		...
		this.apPlain = new folio.editor.AnnotationProfileEditorPlain();
		this.FOAFAPContentPane.attr('content', this.apPlain);
		this.apPlain.setGraph(this.entry.getLocalMetadata(), 'http://tomcat.knowware.nada.kth.se/formulator/formlet/oefoaf');
	},
	_fixCompetenceTab :function(){
			var relations = this.entry.getRelation();
			var relList = relations[this.entry.getResourceUri()];
	...
	clear: function() {
		this.userNameField.attr("value", this.user.name);
		this.passwdField.attr("value", "");
		this.passwdField2.attr("value", "");
		this.languageSelect.setValue(this.user.language);
	},
	...
	changeClicked: function() {
		var newUserData = {name: this.userNameField.attr("value")};
		var pw = this.passwdField.attr("value"); 
		var userLang = this.languageSelect.getValue();
		if (pw) {
			if (pw != this.passwdField2.attr("value")) {
		...
		this.userEntry.getContext().communicator.saveJSON(this.userEntry.getResourceUri(), newUserData,
				dojo.hitch(this, function(data) {
					...
					this.userEntry.getResource().name = newUserData.name;
		...
		var tmp = {metadata : this.apPlain.getGraph().getRoot()};
		this.userEntry.getContext().communicator.saveJSON(this.userEntry.getLocalMetadataUri(),tmp);
		
		//Code below is used only if the user has a Competence Profile (like in H-net)
		if(this.CPContentPane && this.CompetenceEntry){
			var CPGraph = this.CPPlain.getGraph();
			CPGraph.add(this.CompetenceEntry.getResourceUri(), "sc:aboutPerson", {"@id": this.userEntry.getResourceUri()});
			var tmp2 = {metadata : CPGraph.getRoot()};
			this.CompetenceEntry.getContext().communicator.saveJSON(this.CompetenceEntry.getLocalMetadataUri(),tmp2);	
	...
	_setChangeSettingsButtonLabelAttr: function(value) {
		this.changeSettingsButton.attr("label", value);
		this.changeSettingsButtonLabel = value;
	},
	_setCancelButtonLabelAttr: function(value) {
		this.cancelButton.attr("label", value);
